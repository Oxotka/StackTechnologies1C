...
> 🗺️ **[Ландшафт технологий 1С](https://landscape1c.ru)** — интерактивная карта экосистемы технологий 1С: 200+ инструментов, навигация по ролям и контексту работы, фильтры, карточки, граф связей и маршруты освоения. Там информация полнее и удобнее — рекомендую начать с неё.
...
- [Прочие технологии и полезные ссылки](#прочие-технологии-и-полезные-ссылки)
  ...
  - [Прочие технологии](#прочие-технологии)
    ...
    - [Кластер серверов 1С:Предприятие](#кластер-серверов-1спредприятие)
    - [OneSwiss](#oneswiss)
...
S3 — стандарт API для объектных хранилищ (Yandex Object Storage, Amazon S3, MinIO и другие). Файлы хранятся как объекты с уникальными идентификаторами и доступны по единому HTTP-API.

**Зачем нужно:**

Позволяет выносить присоединенные файлы и вложения из базы 1С во внешнее масштабируемое и отказоустойчивое хранилище. Это разгружает базу, упрощает резервное копирование и дает единую точку хранения файлов для разных систем.

**Заметки:**

Готового механизма «из коробки» в платформе нет, но 1С:Предприятие 8.3 умеет выполнять HTTP-запросы, что позволяет работать с S3 через подписанные (pre-signed) ссылки без внешних библиотек. Подход универсален и подходит для разных S3-хранилищ (Yandex Object Storage, Selectel, MinIO и других).

При реализации стоит учитывать пару нюансов: придется доработать механизмы работы с файлами в конфигурации (добавление, изменение, удаление), а основное внимание уделить корректному формированию подписи запроса (`X-Amz-Signature`) — это самое частое место ошибок. Реализация S3 у разных провайдеров может отличаться, поэтому полезно сверяться с официальной документацией конкретного сервиса.

**С чего начать:**

1. Статья - [Из 1С в S3 и обратно. Работа с объектным хранилищем](https://infostart.ru/1c/articles/2033497/)
2. Статья - [Как мы учили «1С:Предприятие» работать с объектным хранилищем S3](https://habr.com/ru/companies/vk/articles/892824/)
3. Расширение [1c-s3connector](https://github.com/ValentinKozlov/1c-s3connector)
...
Кластер серверов — это объединение нескольких рабочих серверов в единый ресурс для обслуживания большого числа одновременных пользователей. Состоит из рабочих процессов (`rphost`), агентов сервера (`ragent`) и менеджеров кластера (`rmngr`).

**Зачем нужно:**

Обеспечивает масштабируемость (наращивание оборудования под нагрузку), отказоустойчивость и балансировку нагрузки между серверами. Понимание устройства кластера важно при работе с серверным вариантом 1С и при администрировании крупных внедрений.

**Подробнее:**

1. Статья - [О кластере серверов 1С:Предприятия](https://habr.com/ru/companies/1c/articles/493008/)
2. Официальный сайт - <https://v8.1c.ru/platforma/klaster-serverov/>

[В начало](#стек-технологий-для-1с)

### OneSwiss

Кроссплатформенный комплекс для обслуживания и мониторинга информационных баз 1С. Состоит из серверного приложения с веб-интерфейсом и агента, работающего на серверах 1С.

![OneSwiss](img/OneSwiss.png "OneSwiss")​

**Зачем нужно:**

Автоматизирует регламентное обслуживание баз и упрощает работу администратора: блокировка соединений, загрузка конфигураций и расширений, запуск внешних обработок и скриптов по шаблонам в виде блок-схем.

**Заметки:**

Помимо обслуживания, умеет собирать и экспортировать технологические журналы в ClickHouse, выгружать исходники конфигураций в Git-репозитории, проксировать публикацию хранилищ конфигураций, регистрировать ошибки и отправлять уведомления в Telegram и по WebHook.
