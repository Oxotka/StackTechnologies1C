...
- Быстро интегрируется в процесс разработки
- Не требует фундаментального пересмотра философии программирования, но дает возможность применять TDD
...
Повышает качество кода, позволяет разрабатывать и запускать тесты, автоматизировать рутинные операции. На нем возможно писать юнит, интеграционные и сценарные тесты. 

* Юнит-тест (unit test), или модульный тест,  - это проверка работу небольшой части кода. Отдельных процедур, функций.
* Интеграционное тестирование – это тестирование интеграций. Интеграция – это любое взаимодействие отдельный баз / конфигураций / сервисов. Например: мобильное приложение - основная база, основная база - кафка, основная база - API ЦБ.
* Сценарное тестирование – оно же End-to-end (e2e), или Сквозное тестирование - это методика, которая строится на активном применении пользовательских сценариев — последовательностей действий пользователя в приложении. Например, симуляция процесса закупа товара: заказ у поставщика, оплата, оприходование.

**С чего начать:**

**1. Старт в EDT**
>
>  Т.к. на которые действия по установки могут изменять - прикладываю инструкции на первоисточник.
>
>  #### Шаг 1. Скачиваем расширение для тестирования.
>  
>  * [Git YAxUnit](https://github.com/bia-technologies/yaxunit)
>   * [Начало работы - документация YAxUnit](https://bia-technologies.github.io/yaxunit/docs/getting-started/)
>   * [Руководство - документация YAxUnit](https://bia-technologies.github.io/yaxunit/docs/features/)
>
>  #### Шаг 2. Скачиваем плагин EDT и устанавливаем его.
>  * [Плагин для EDT](https://github.com/bia-technologies/edt-test-runner)
>
>  Переходим в нужный проект и устанавливаем плагин. 2 инструкции об установки плагина, описывают одно и тоже, но выглядят чуть-чуть по разному.
>
>  * [Установка плагина в EDT- документация YAxUnit](https://bia-technologies.github.io/edt-test-runner)
>  * [Установка плагина в EDT - документация плагина](https://github.com/bia-technologies/edt-test-runner/blob/develop/docs/Установка.md)
>
>  Проверить что все встало корректно  можно с помощью панели окн.
  ![Добавить Окно Модульные Тесты](img/YAxUnit/ДобавитьОкноМодульныеТесты.png "Добавить Окно Модульные Тесты")
>
>  Тыкаем в EDT после установки:
>
>  1. Окно
>  2. Показать панель
>  3. Другое
>  4. Ищем в папке "1С: Предприятие" "1С:Модульные тесты". Если есть, то всё отлично.
>
>  #### Шаг 3. Добавляем в проект проект расширения. 
>  Заходим в конфигуратор, заливаем в него расширение скаченное на шаге 1, убираем галки безопасности, возвращаемся в EDT и импортируем расширение в проект и подливаем в репозиторий. 
>
>  #### Шаг 4. Добавляем новый тип конфигурации запуска. 
>  Он нужен, чтобы мы могли быстро запускать тесты и смотреть их результат в EDT.
>
>  ![Настройки Запуска Конфигурации](img/YAxUnit/НастройкиЗапускаКонфигурации.png "Настройки Запуска Конфигурации")
>
>  1. Запуск
>  2. Конфигурация запуска...
>  3. Смотрим, что есть базовый режим запуска. Если нет то создаём его. 
>  4. Жмем ПКМ по строке "модульное тестирование 1С:..."
>  5. Настраиваем синоним запуска
>  6. Выбираем базовый клиент, под которым будет первый запуск
>  7. Выбираем наше расширение с YAxUnit тестами
>
> #### Шаг 5. Запуск.
>Когда режимы готовы, сначала запускаем базовый (1), а потом для тестирования (2). При дальнейших запусках, можно сразу запускать тестирование (2).
>
>![Запуск Конфигурации](img/YAxUnit/ЗапускКонфигурации.png "Запуск Конфигурации")
>
>Во время запуска тестирования: будет открыто предприятие, пройдут тесты, предприятие закроется и будет сформирован результат тестирования. 
>
>![Результат Теста](img/YAxUnit/РезультатТеста.png "Результат Теста")

**2. Старт в конфигураторе**
>
>  * Шаг 1. Скачиваем расширение для тестирования.
>
>    * [Git YAxUnit](https://github.com/bia-technologies/yaxunit)
...
>    * [Руководство - документация YAxUnit](https://bia-technologies.github.io/yaxunit/docs/features/)
>
>  * Шаг 2. Добавляем расширение в конфигурацию. Убираем галки безопасности.
>  * Шаг 3. [Пишем тест](https://bia-technologies.github.io/yaxunit/docs/getting-started/first-test)
>  * Шаг 4. Запускам предприятие - переходим в подсистему YAxUnit - Запуск тестов 
>    Тут в таблице мы должны увидеть тест, которые написали в шаге 3. После, запускаем тесты и смотрим, как они прошли. 

**Подробнее:**

1. Исходный код - <https://github.com/bia-technologies/yaxunit>
2. Документация - <https://bia-technologies.github.io/yaxunit/>
3. Плагин для EDT - <https://github.com/bia-technologies/edt-test-runner>
4. Документация плагина - <https://bia-technologies.github.io/yaxunit/docs/getting-started/edt-plugin/> 
5. Установка плагина - <https://github.com/bia-technologies/edt-test-runner/blob/develop/docs/Установка.md>
6. Группа в телеграм - <https://t.me/BIAOpenTools/12>
